    ...
    auto thread_func = [&]
    {
        ...
        for (size_t i = 0; i < num_keys * passes; i += step)
        {
            ...
            if (key_detached_if_missing)
                CHECK(!chunk.chunk()->pages_populated.any());

            for (size_t page_idx = 0; page_idx < pages_per_chunk; ++page_idx)
            {
                bool populated = chunk.chunk()->pages_populated.get(page_idx);
                ...
                else
                {
                    ...
                    auto items = reinterpret_cast<std::atomic<size_t> *>(chunk.chunk()->data + cache.pageSize() * page_idx);
                    if (populated)
                    {
    ...
    for (size_t num_threads = 1; num_threads <= 16; num_threads *= 2)
    {
        for (size_t run = 0; run < 8; ++ run)
        {
            ...
            for (size_t i = 0; i < cache.maxChunks(); ++i)
            {
                ...
                memset(chunk.chunk()->data, 42, chunk.chunk()->size);
