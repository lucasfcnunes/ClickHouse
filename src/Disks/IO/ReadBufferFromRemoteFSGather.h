...
public:
    ...
    /// nextImpl() call, or when readInto() is always used instead of nextImpl()) and owned buffer mode.
    /// Automatically detects which one to use (lazily allocates own memory when nextImpl() is called
    /// with no buffer assigned).
    ...
    /// If all reads are done using this method, with no nextImpl() calls, then we never allocate a buffer.
