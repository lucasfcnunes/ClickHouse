...
{
    seek(static_cast<off_t>(offset), SEEK_SET);

    if (available() != 0)
    {
        IAsynchronousReader::Result res;
        res.size = std::min(size, available());
     ...
     */
    Buffer original_buffer = internal_buffer;
    set(data, size);
    SCOPE_EXIT({
        set(original_buffer.begin(), original_buffer.size());
    });
    ...
    chassert(data == working_buffer.begin());
    chassert(size >= working_buffer.size());
