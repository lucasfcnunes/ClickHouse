...
private:
    ...
    /// Detached chunks may still be pinned. Chunk may get deteched even while pinned, in particular when dropping cache.
...
private:
    ...
    PageChunk * chunk_ = nullptr;

    PinnedPageChunk(PageCache * cache, PageChunk * chunk) noexcept;
};
...
private:
    ...
    struct Mmap
    {
        ...
        size_t num_chunks; // might be smaller than chunks_per_mmap_target because of alignment
    ...
    void sendChunkToLimbo(PageChunk * chunk, std::unique_lock<std::mutex> & /* chunk_mutex */) noexcept;
