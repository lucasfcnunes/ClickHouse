...
{
    if (cache)
        cache->removeRef(chunk_);
    cache = std::exchange(c.cache, nullptr);
    chunk_ = std::exchange(c.chunk_, nullptr);
    return *this;
}
...
PinnedPageChunk::PinnedPageChunk(PageCache * _cache, PageChunk * chunk) noexcept : cache(_cache), chunk_(chunk) {}

const PageChunk * PinnedPageChunk::chunk() const { return chunk_; }

bool PinnedPageChunk::markPagePopulated(size_t page_idx)
{
    bool r = chunk_->pages_populated.set(page_idx);
    return r;
}

void PinnedPageChunk::markPrefixPopulated(size_t bytes)
{
    for (size_t i = 0; i < (bytes + chunk_->page_size - 1) / chunk_->page_size; ++i)
        markPagePopulated(i);
}
...
{
    for (size_t i = 0; i < (bytes + chunk_->page_size - 1) / chunk_->page_size; ++i)
        if (!chunk_->pages_populated.get(i))
            return false;
    return true;
}
...
bool AtomicBitSet::set(size_t i)
{
    UInt8 prev = v[i / 8].fetch_or(1 << (i % 8));
    return (prev & (1 << (i % 8))) == 0;
}

bool AtomicBitSet::set(size_t i, bool val)
{
    if (val)
        return set(i);
    else
...
bool AtomicBitSet::unset(size_t i)
{
    UInt8 prev = v[i / 8].fetch_and(~(1 << (i % 8)));
    return (prev & (1 << (i % 8))) != 0;
}

void AtomicBitSet::unsetAll()
{
    memset(&v[0], 0, (n + 7) / 8);
}
...
void PageCache::sendChunkToLimbo(PageChunk * chunk, std::unique_lock<std::mutex> & /* chunk_mutex */) noexcept
{
...
std::pair<size_t, size_t> PageCache::restoreChunkFromLimbo(PageChunk * chunk, std::unique_lock<std::mutex> & /* chunk_mutex */) noexcept
{
    ...
    for (size_t idx = 0; idx < chunk->size / bytes_per_page; ++idx)
    {
        ...
        ///        Set the second byte to 0, in case the page was freed between steps 1 and 2.
...
PageCache::Mmap::Mmap(Mmap && m) noexcept : ptr(std::exchange(m.ptr, nullptr)), size(std::exchange(m.size, 0)), chunks(std::move(m.chunks)) {}
